 ...
 * @description: AbstractSipRequestHandlerMethodMapping
 **/
public abstract class AbstractSipRequestHandlerMethodMapping<T> extends AbstractSipHandleMapping implements InitializingBean  {
    ...
    private static final HandlerMethod PREFLIGHT_AMBIGUOUS_MATCH =
            new HandlerMethod(new AbstractSipRequestHandlerMethodMapping.EmptyHandler(), ClassUtils.getMethod(AbstractSipRequestHandlerMethodMapping.EmptyHandler.class, "handle"));
    ...
    protected void detectHandlerMethods(Object handler) {
        ...
        if (handlerType != null) {
            ...
            methods.forEach((method, mapping) -> {
                ...
                registerHandlerMethod(createBeanInstance(handlerType), invocableMethod, mapping);
            });
        }
    }
    ...
    private Object createBeanInstance(Class<?> cls) {
        try {
            String[] names = obtainApplicationContext().getBeanNamesForType(cls);
            if (names.length != 0) {
                return obtainApplicationContext().getBean(cls);
            }
            return cls.getDeclaredConstructor().newInstance();
        }catch (Exception ex){
            logger.error("createBeanInstance error :"+ex.getMessage());
            return null;
        }
    }
